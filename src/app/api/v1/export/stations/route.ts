...
export async function GET() {
  ...
  const { data, error } = await createServiceRoleClient().from('charging_stations').select('code,name,x,y,address,is_active').order('code');
  if (error) return NextResponse.json({ error: { message: error.message } }, { status: 500 });
  const body = csv([['Mã trạm', 'Tên trạm', 'X', 'Y', 'Địa chỉ', 'Đang hoạt động'], ...(data ?? []).map((row: any) => [row.code, row.name, row.x, row.y, row.address, row.is_active ? 'Có' : 'Không'])]);
  return new NextResponse(body, { headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': 'attachment; filename="danh-muc-tram-sac.csv"' } });
}
