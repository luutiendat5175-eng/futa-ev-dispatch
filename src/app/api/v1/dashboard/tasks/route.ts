...
export async function GET() {
  ...
  if (!plan) return NextResponse.json({ tasks: [], plan: null });
  const { data: tasks, error } = await db.from('dispatch_tasks')
    .select('*, assigned_profile:profiles!dispatch_tasks_assigned_profile_id_fkey(full_name,employee_code), vehicles(license_plate), daily_vehicle_schedules(earliest_departure_at, plan_route_ends(route_code,route_end_name))')
  ...
  if (!['lai_xe', 'dieu_phoi'].includes(actor.role)) return NextResponse.json({ tasks: tasks ?? [], plan });
  ...
  return NextResponse.json({ tasks: (tasks ?? []).filter((task: { to_station_id: string | null; assigned_profile_id: string | null }) => Boolean((task.to_station_id && permitted.has(task.to_station_id)) || task.assigned_profile_id === actor.userId)), plan });
}
