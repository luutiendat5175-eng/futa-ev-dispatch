  ...
  try {
    ...
    if (!plan && !planError) {
      ...
      if (snapshot.data) { plan = snapshot.data as { id: string; status: string }; createdNewPlan = true; }
        ...
        else return NextResponse.json({ error: { message: snapshot.error?.message === 'NO_ACTIVE_OVERNIGHT_CONFIG' ? 'Chưa có PA đậu đêm cố định đang hiệu lực. Hãy import PA trước.' : snapshot.error?.message ?? 'Không thể tạo snapshot PA cho ngày vận doanh.' } }, { status: 409 });
      }
    }
