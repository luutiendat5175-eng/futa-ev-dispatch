...
export function QrScanner({ onScan, onClose }: QrScannerProps) {
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          // A capped frame is materially faster to decode on mid-range phones
          // than using the camera's native 4K frame for every scan attempt.
          video: { facingMode: { ideal: 'environment' }, width: { ideal: 960 }, height: { ideal: 960 } },
        });
    ...
    function tick() {
      ...
      if (performance.now() - lastDecodeAt < 120) { animationFrameId = requestAnimationFrame(tick); return; }
