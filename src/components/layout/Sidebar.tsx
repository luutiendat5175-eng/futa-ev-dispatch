...
import {
  getSidebarMenuForRole,
  type SidebarMenuItem,
} from "@/domain/layout/sidebarMenu";
  ...
  useEffect(() => {
    authFetch("/api/v1/me")
      ...
      .then((data) => {
        ...
        const nextMenu = getSidebarMenuForRole(data.role);
        setMenu(nextMenu);
        nextMenu.forEach((item) => router.prefetch(item.href));
      })
      .catch(() => undefined);
  }, []);
  ...
  return (
    <aside className="app-sidebar">
      <div className="sidebar-brand">
        <strong>FUTA - EV DISPATCH</strong>
        <span>Điều phối di dời xe</span>
      </div>
      ...
      {userInfo && (
        <div className="sidebar-user">
          ...
          <span>{ROLE_LABEL[userInfo.role]}</span>
        </div>
      )}
    </aside>
  );
