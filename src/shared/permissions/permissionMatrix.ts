...
const MATRIX: Record<PermissionAction, Record<UserRole, boolean>> = {
  ...
  // "Toàn bộ" nghĩa là không bị giới hạn theo trạm. Điều phối và lái xe vẫn
  // xem bản đồ, nhưng API chỉ trả về các trạm đã được phân công.
  xuat_danh_sach_xe: { admin: true, dieu_phoi: true, dieu_do: true, lai_xe: false },
  xem_toan_bo_map: { admin: true, dieu_phoi: false, dieu_do: true, lai_xe: false },
  tao_dieu_phoi_task: { admin: true, dieu_phoi: true, dieu_do: false, lai_xe: false },
  // Chỉ lái xe tạo bằng chứng GPS/ảnh và thực hiện giao nhận. Admin có thể
  // rollback để sửa sai, nhưng không được giả thao tác vận hành thay lái xe.
  cap_nhat_trang_thai_task: { admin: false, dieu_phoi: false, dieu_do: false, lai_xe: true },
  // Điều phối được rollback nhưng RPC còn kiểm tra task phải thuộc trạm mà
  // điều phối được phân công. Admin vẫn có thể rollback mọi trạm.
  rollback_trang_thai: { admin: true, dieu_phoi: true, dieu_do: false, lai_xe: false },
  chup_anh_doi_soat: { admin: false, dieu_phoi: false, dieu_do: false, lai_xe: true },
  ...
  cau_hinh_tham_so_lct: { admin: true, dieu_phoi: false, dieu_do: false, lai_xe: false },
