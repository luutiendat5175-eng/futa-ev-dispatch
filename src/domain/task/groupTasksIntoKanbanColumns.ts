...
export interface KanbanTask { id: string; loaiTask: TaskType; trangThai: string; eta: string | null; priorityLctAt?: string | null; vehicleId: string | null; vehiclePlate?: string | null; routeCode?: string | null; departureAt?: string | null; assignedUserId: string | null; }
export interface KanbanColumn { status: string; label: string; tasks: KanbanTask[]; }
const MOVE_COLUMNS: [string, string][] = [['chua_sac','Chưa nhận'],['nhan_xe_dau_ben','Đã nhận'],['giao_tram_sac','Giao trạm sạc'],['nhan_tram_sac','Trả xe'],['hoan_thanh','Hoàn thành']];
