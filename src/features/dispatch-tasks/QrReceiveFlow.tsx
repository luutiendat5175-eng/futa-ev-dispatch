...
export function QrReceiveFlow({ actor }: { actor: { userId: string; role: UserRole; fullName: string } | null }) {
  const [open, setOpen] = useState(false); const [task, setTask] = useState<Task | null>(null); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false);
  const scan = async (payload: { vehicleId: string }) => { setOpen(false); setMessage('Đang kiểm tra task…'); const response = await authFetch(`/api/v1/qr/vehicle-task?vehicleId=${encodeURIComponent(payload.vehicleId)}`); const data = await response.json(); if (!response.ok) { setTask(null); setMessage(data.error?.message ?? 'Không tìm thấy task.'); return; } setTask(data.task); setMessage(''); };
  const confirm = async () => { if (!task || !actor || task.status !== 'chua_sac' || task.assignedUserId) return; setBusy(true); const response = await authFetch(`/api/v1/tasks/${task.id}/claim`, { method: 'POST' }); const data = await response.json(); if (!response.ok) setMessage(data.error?.message ?? 'Không thể nhận xe.'); else setTask({ ...task, assignedUserId: actor.userId, status: data.task?.status ?? task.status }); setBusy(false); };
  return <div className="qr-flow"><button className="qr-scan-button" onClick={() => { setTask(null); setMessage(''); setOpen(true); }}>Quét QR nhận xe</button>{open && <QrScanner onScan={scan} onClose={() => setOpen(false)} />}{message && <span>{message}</span>}{task && <div className="qr-task-modal"><div><button className="qr-close" onClick={() => setTask(null)}>×</button><p className="workspace-eyebrow">XÁC NHẬN XE</p><div className="qr-plate-line"><h2>{task.licensePlate}</h2><b className="qr-priority">Ưu tiên {task.priority ?? '—'}</b></div><p>Tuyến {task.routeCode} · {task.routeEndName}</p><p>STT bảng tài: <b>{task.sequence ?? '—'}</b> · Giờ xuất bến: <b>{time(task.departureAt)}</b></p>{task.status === 'chua_sac' && !task.assignedUserId ? <div className="qr-confirm-actions"><button className="qr-scan-button" disabled={busy} onClick={confirm}>{busy ? 'Đang xác nhận…' : 'Xác nhận nhận xe'}</button><button onClick={() => { setTask(null); setOpen(true); }}>Hủy / quét lại</button></div> : task.assignedUserId === actor?.userId ? <TaskProofAction taskId={task.id} status={task.status} assignedUserId={task.assignedUserId} actor={actor} vehiclePlate={task.licensePlate} startPhotoPicker /> : <p>Task đã được nhận bởi nhân viên khác.</p>}</div></div>}</div>;
}
