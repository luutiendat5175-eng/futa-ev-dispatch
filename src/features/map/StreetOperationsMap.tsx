...
type Staff = {
  ...
  stationId?: string;
  depotId?: string;
  kind: "outbound" | "station" | "inbound" | "depot";
  isLate?: boolean;
  dueAt?: string;
};
type LeafletMap = {
...
type LeafletApi = {
  ...
  circleMarker: (
    ...
    addTo: (layer: LeafletLayerGroup) => { bindPopup: (html: string) => void };
  };
  polyline: (
...
function parallelPath(
  depot: YardLocation,
  station: YardLocation,
  side: 1 | -1,
  outbound: boolean,
): [number, number][] {
  const latitudeDelta = station.x - depot.x;
  const longitudeDelta = station.y - depot.y;
  const length = Math.hypot(latitudeDelta, longitudeDelta) || 1;
  const shift = 0.0011;
  const latitudeOffset = (-longitudeDelta / length) * shift * side;
  const longitudeOffset = (latitudeDelta / length) * shift * side;
  const depotPoint: [number, number] = [
    depot.x + latitudeOffset,
    depot.y + longitudeOffset,
  ];
  const stationPoint: [number, number] = [
    station.x + latitudeOffset,
    station.y + longitudeOffset,
  ];
  return outbound ? [depotPoint, stationPoint] : [stationPoint, depotPoint];
}

export function StreetOperationsMap({
  depots,
  stations,
  staffLocations,
}: {
  ...
  useEffect(() => {
    ...
    void loadLeaflet()
      ...
      .catch(() => {
        if (element.current)
          element.current.textContent = "Không tải được bản đồ đường phố.";
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    const all = [...depots, ...stations];
    const ids = new Map(all.map((item) => [item.id, item]));
    const detail = (rows: Staff[]) =>
      rows
        .slice()
        .sort((a, b) => b.occurredAt.localeCompare(a.occurredAt))
        .map(
          (item) =>
            `${item.vehiclePlate} · ${item.employeeName} · <span style="color:${item.isLate ? "#dc2626" : "#111827"};font-weight:${item.isLate ? "700" : "400"}">${clock(item.occurredAt)}</span>`,
        )
        .join("<br/>") || "Không có nhân viên";
    for (const item of all) {
      const isStation = stations.some((station) => station.id === item.id);
      const rows = staffLocations.filter((staff) =>
        isStation
          ? staff.kind === "station" && staff.stationId === item.id
          : staff.kind === "depot" && staff.depotId === item.id,
      );
      L.circleMarker([item.x, item.y], {
        radius: 10,
        color: isStation ? "#c2410c" : "#0f766e",
        fillOpacity: 0.9,
      })
        .addTo(layers.current)
        .bindPopup(`<b>${item.name}</b><br/>${detail(rows)}`);
    }
    const routes = new Map<string, Staff[]>();
    for (const staff of staffLocations.filter(
      (item) => item.kind === "outbound" || item.kind === "inbound",
    )) {
      if (!staff.depotId || !staff.stationId) continue;
      const key = `${staff.kind}:${staff.depotId}:${staff.stationId}`;
      routes.set(key, [...(routes.get(key) ?? []), staff]);
    }
    for (const [key, routeStaff] of routes) {
      const [kind, depotId, stationId] = key.split(":");
      const depot = ids.get(depotId);
      const station = ids.get(stationId);
      if (!depot || !station) continue;
      const outbound = kind === "outbound";
      const points = parallelPath(depot, station, outbound ? 1 : -1, outbound);
      const title = outbound
        ? `Chiều đi: ${depot.name} → ${station.name}`
        : `Chiều về: ${station.name} → ${depot.name}`;
      L.polyline(points, {
        color: outbound ? "#16a34a" : "#dc2626",
        weight: 6,
        dashArray: outbound ? "" : "12 9",
        opacity: 0.9,
      })
        .addTo(layers.current)
        .bindPopup(`<b>${title}</b><br/>${detail(routeStaff)}`);
    }
