...
export function AttendanceScreen() {
  const [sessions, setSessions] = useState<Session[]>([]); const [actor, setActor] = useState(''); const [actorName, setActorName] = useState(''); const [q, setQ] = useState(''); const [notice, setNotice] = useState(''); const [busy, setBusy] = useState(false); const [photos, setPhotos] = useState<File[]>([]); const input = useRef<HTMLInputElement>(null);
  ...
  return <main className="attendance-page"><header><div><h1>Chấm công</h1><p>Vào/ra ca bắt buộc có ảnh, thời gian và định vị GPS.</p></div><button className="primary" disabled={busy} onClick={() => void submit(active ? 'out' : 'in')}>{busy ? 'Đang ghi nhận…' : active ? 'Ra ca' : 'Vào ca'}</button></header><div className="attendance-tools"><input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Tìm tên hoặc MSNV…"/><button onClick={() => exportCsv(sessions)}>Xuất dữ liệu</button></div><div className="attendance-photo"><input ref={input} type="file" multiple accept="image/jpeg,image/png,image/webp" capture="environment" onChange={(event) => setPhotos([...photos, ...Array.from(event.target.files ?? [])])}/><span>{photos.length ? `Đã chọn ${photos.length} ảnh` : 'Chọn ảnh chấm công (ít nhất 1)'}</span></div>{notice && <p className="attendance-notice">{notice}</p>}<section className="attendance-summary"><strong>{active ? 'Đang trong ca' : 'Chưa vào ca'}</strong><span>{active ? `Bắt đầu ${time(active.checked_in_at)}` : 'Chọn ảnh, bật GPS rồi bấm “Vào ca”.'}</span></section><section className="attendance-list"><h2>Lịch sử chấm công</h2>{sessions.map((session) => <article key={session.id}><strong>{session.profiles?.full_name ?? '—'} <small>· {session.profiles?.employee_code}</small></strong><span>Vào: {time(session.checked_in_at)}</span><span>{session.checked_out_at ? `Ra: ${time(session.checked_out_at)}` : 'Đang làm việc'}</span></article>)}{!sessions.length && <p>Chưa có phiên làm việc.</p>}</section></main>;
}
