-- Keep the mobile workflow as one action: "Nhận xe trạm sạc" -> "Trả xe".
-- This replaces older function variants that still required giao_dau_ben.
...
begin
  ...
  if taking_return then select id into other_active_task from public.dispatch_tasks where assigned_profile_id=auth.uid() and task_type='di_chuyen' and id<>task.id and status in('nhan_xe_dau_ben','nhan_tram_sac') limit 1 for update;if other_active_task is not null then raise exception 'EMPLOYEE_ACTIVE_TASK';end if;end if;
  ...
  insert into public.task_events(task_id,event_type,from_status,to_status,actor_profile_id,latitude,longitude,gps_accuracy_m,note)values(task.id,'transition',task.status,p_next_status,auth.uid(),p_latitude,p_longitude,p_accuracy_m,p_note)returning * into created_event;return created_event;
end $$;
