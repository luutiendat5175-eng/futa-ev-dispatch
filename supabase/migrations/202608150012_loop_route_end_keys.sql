-- A circular route can have two operational blocks with the same displayed
-- terminal name.  Keep the display name, but give each block an internal key
-- (A/B) so routing, LCT and roster imports never merge them.
