-- Restore the one-step return flow used by the current UI and retain the
-- legacy two-step return path for historical tasks.
...
begin
  ...
  if task.task_type='di_chuyen' then
    allowed:=(task.status,p_next_status) in (
      ('chua_sac','nhan_xe_dau_ben'),('nhan_xe_dau_ben','giao_tram_sac'),('giao_tram_sac','nhan_tram_sac'),
      ('nhan_tram_sac','hoan_thanh'),('nhan_tram_sac','giao_dau_ben'),('giao_dau_ben','hoan_thanh')
    );
  else
  ...
  insert into public.task_events(task_id,event_type,from_status,to_status,actor_profile_id,latitude,longitude,gps_accuracy_m,note)
    values(task.id,'transition',task.status,p_next_status,auth.uid(),p_latitude,p_longitude,p_accuracy_m,p_note) returning * into created_event;
